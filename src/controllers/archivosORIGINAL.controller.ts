...
export class ArchivosController {
  ...
  async fileUploading(
    ...
    const filePath = path.join(__dirname, GeneralConfig.carpetaArchivosUsuarios,
    )
    let res = await this.StoreFileToPath(
      filePath,
      GeneralConfig.campoDeUsuario,
      request,
      response,
  ...
  async downloadFileByName(
    ...
    // console.log('get | ObtenerArchivo/{type}/{name}:', response)
    return response
  }
  ...
  private GetFolderPathByType(type: number) {
    ...
    switch (type) {
      ...
      case 1:
        ...
        break
      case 2:
        filePath = path.join(__dirname, GeneralConfig.carpetaArchivosCobradores)
        break
      case 3:
        filePath = path.join(__dirname, GeneralConfig.carpetaArchivosInversores)
        break
    }
    return filePath
  }
